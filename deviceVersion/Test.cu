...
class TestQR_7 {
public:
	TestQR_7(cuComplex* d_s, cuComplex* d_y,
		cuComplex* t_conj,
		cuComplex* d_u) {
		cuComplex* temp_s;
		cuComplex* temp_y;
		cudaMalloc((void**)&temp_s, sizeof(cuComplex)*M*N*M*N);
		cudaMalloc((void**)&temp_y, sizeof(cuComplex)*M*N*M*N);

		for (int i = 0; i < BIN; i++) {
			matrixMulC(temp_y, d_y + i*(M*N), d_y + i*(M*N), M*N, M*N, L, M*N, M*N, M*N, CUBLAS_OP_N, CUBLAS_OP_C);
			subtraction(temp_s, d_s, temp_y, M*N, M*N, M*N, M*N);
			choleskySolver(d_u+i*(M*N), temp_s, t_conj, M*N, M*N, M*N);
		}
...
int main() {	
	...
	for (int testN = 0; testN < TESTTIMES; testN++) {
		outFile << "Test begin with "<<BIN<<" bins!\n";
		...
		TestQR_7 *test7 = new TestQR_7(d_s, d_y, d_t, d_u);
		delete test7;
	...
	printf("Test finished with %d x %d x %d cube with %d bins!\n",M,N,L,BIN);
	//system("pause");
	return 0;
}
